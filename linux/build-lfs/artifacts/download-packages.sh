...
# Uses GNU mirror servers and retry logic for reliability
...
echo "=== Downloading LFS 12.4 Packages ==="
echo "Target directory: $SOURCES"
echo ""

mkdir -p $SOURCES
cd $SOURCES

# Use ftpmirror.gnu.org (better availability) instead of ftp.gnu.org
# Create wget-list with mirror URLs
cat > wget-list << 'EOF'
...
https://ftpmirror.gnu.org/gettext/gettext-0.26.tar.xz
https://ftpmirror.gnu.org/glibc/glibc-2.42.tar.xz
...
https://ftpmirror.gnu.org/inetutils/inetutils-2.6.tar.xz
...
echo "Downloading source packages (with retry)..."
# Use retry options for more reliable downloads
wget --input-file=wget-list \
    ...
    2>&1 | tee download.log || true

echo ""
echo "Downloading patches..."
wget --input-file=patch-list \
    --continue \
    --directory-prefix=$SOURCES \
    --tries=3 \
    --timeout=30 \
    2>&1 | tee -a download.log || true

echo ""
echo "=== Download Summary ==="
total=$(wc -l < wget-list)
patches=$(wc -l < patch-list)
downloaded=$(ls -1 *.tar.* *.patch 2>/dev/null | wc -l)
echo "Expected packages: $total"
echo "Expected patches: $patches"
echo "Downloaded files: $downloaded"

if [ "$downloaded" -ge "$((total + patches - 5))" ]; then
    echo ""
    echo "SUCCESS: Most packages downloaded!"
else
    echo ""
    echo "WARNING: Some downloads may have failed. Check download.log"
fi
...
echo ""
echo "Done! Sources are in $SOURCES"
